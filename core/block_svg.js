...
Blockly.BlockSvg.prototype.onMouseMove_ = function(e) {
  ...
  if (Blockly.dragMode_ == 2) {
    ...
    if (Blockly.highlightedConnection_ &&
      ...
      if (source.getNextStatementInput()) {
    ...
    if (closestConnection &&
      ...
      // get the block the highlighted connection originates from
      var source = Blockly.highlightedConnection_.sourceBlock_;
      var sourceInput = source.getNextStatementInput();
  ...
  if (this.disabled || this.getInheritedDisabled()) {
    if (!hasClass) {
      ...
      this.svgPath_.setAttribute('fill',
          'url(#' + this.workspace.getOption("disabledPatternId") + ')');
    }
  } else {
    if (hasClass) {
      Blockly.removeClass_(/** @type {!Element} */ (this.svgGroup_),
...
Blockly.BlockSvg.prototype.renderDrawBottom_ =
  ...
  if (this.squareBottomLeftCorner_) {
    ...
    if (!this.RTL) {
      ...
      this.heightInfoListLight.push({index:highlightSteps.length-1, prefix:'0.5,', original:cursorY - 0.5});
    }
  } else {
    ...
    if (!this.RTL) {
      ...
      // store info about light path on the outside left edge for resizing
      this.heightInfoListLight.push({index:highlightSteps.length-1, prefix:(Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ',' +
          (Blockly.BlockSvg.CORNER_RADIUS - 0.5) + ' 0 0,1 ' + '0.5,', original:cursorY - Blockly.BlockSvg.CORNER_RADIUS});
    }
  }
};
